...
* 所有图像均被表示为二维数组( vector ) $A_{N\times N},A_{i,j}\in [0,1]$ ，其中 1 为纯白，0 为纯黑。
* 所有图像都由 png/ppm 格式储存。
* 数字都为白色，且符合人类正常书写规律。不会出现倒着写，横着写的情况。
* 保证数字一定存在于图片中，但不对数字的形状、大小、区域做任何保证。
* 我们下发了 100 张图片用于你来分析和设计算法，详见仓库中的 `data/` 与 `png_data/` ，数据具体来源及与测试数据集差别详见 tips。


### Prerequisite

当然，你可能想自己写一些代码对图片进行分析，这里提供了一些可能有用的帮助：

* 由于使用 C++ 读入 png/jpg/jpeg 图片非常不方便，我们下发了 ppm 格式的图片以方便读取，在 `data/` 目录下。
...
* 源文件 `read_support.cpp` 与 `read_support.hpp` 已下发在仓库中。
`read_support.hpp` 中提供了两个函数的接口：
...
**为了方便更有针对性的优化**，在评测信息中给出了 $10\times 10$ 的矩阵 $Acc$ ，其中 $Acc_{i,j}$ (0-based) 为你在 label $i$ 的测试数据上的预测为 label $j$ 的概率（ $\sum_{j=0}^9 Acc_{i,j}=1$ ）。请在评测信息中点击 `show detail` 进行查看。
...
* 评测虽然只允许提交 C++ ，但本地用其他语言进行测试和分析是完全可以的。
